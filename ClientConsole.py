...
class Placebid():
    def __init__(self,tb,order_type,amnt,waktu,refid):
        self.uagent = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.14; rv:97.0) Gecko/20100101 Firefox/97.0"
        ...
        self.headers = {'User-Agent': self.uagent}
        ...
        self.history=[]; self.lastSend=time.time(); self.ref=refid; self.stop=False
        ...
        self.typeBid(tb,order_type,amnt,waktu)
    ...
    def customBid(self, status, amount, timees=5):
        bidTime=self.parseBidTime(int(timees))
        self.sendWs('{"topic":"base","event":"create_deal","payload":{"amount":'+str(amount*100)+',"asset":"'+self.assetRic+'","asset_id":'+str(self.assetId)+',"asset_name":"'+self.currency+'","created_at":'+str(int(time.time()))+',"currency_iso":"IDR","deal_type":"'+self.walletType+'","expire_at":'+bidTime+',"option_type":"turbo","tournament_id":null,"trend":"'+status+'","is_state":false},"ref":"~~","join_ref":"~~"}')
