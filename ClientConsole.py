...
class Placebid():
    def __init__(self):
        ...
        self.url_server = self.settings["url_server"]
    ...
    def bid(self, tb, order_type, amnt, waktu):
        ...
        losbet = int(abs(self.getLossBet()))        
        print("bid :: last_bet_amount -> ", deal_amount)
        print("bid :: losbet -> ", int(losbet / 100))
        print("deal_amount < amount -> ", int(deal_amount) > int(self.amount))
        div_loss = self.round_up(losbet / 100)
        deal_amount = abs(self.round_up((div_loss + self.amount) * self.multi_bet_amount))
        deal_amount = self.amount if deal_amount < self.amount else deal_amount
            ...
            try:
                ...
                if len(data['event']) > 0:
                    if data["event"] == "deal_created":
                        ...
                        if (losbet / 100) <= 0:
                            self.setLossBet(0)
                            self.setBetAmount(self.amount * self.multi_bet_amount)
                        elif losbet < self.amount :                            
                            setbet = self.amount + losbet
                            self.setBetAmount(setbet)   
                        elif losbet > self.amount and losbet < self.amount * self.max_loss_level:
                            setbet = int(losbet / 100)  * self.multi_bet_amount                          
                            self.setBetAmount(setbet)   
                        elif (self.getCountLoss() >= self.max_loss_level) or (losbet > self.amount * self.max_loss_level):
                            self.setCountLoss(0)
                            betamount = (losbet / 100) / self.max_loss_level
                            self.setBetAmount(betamount)
                        else:
                            setbet = int(((losbet / 100) + self.amount) * self.multi_bet_amount)
                            self.setBetAmount(setbet)

                        # bet history
                        result = self.df_martingale.to_json(orient="table")
                        parsed = json.loads(result)
                        txtto = json.dumps(parsed)
                        self.sendtoServer(txtto)
                        self.df_martingale.to_csv("bet_result.csv")

                        print("total loss -> ", losbet / 100)
                        print("new bet amount -> ", betamn)
                        print("new martingale -> ", self.df_martingale.tail(10))
                        ...
                        print("df_martingale -> ", self.df_martingale)
    ...
    def setOpenPos(self, data):
        self.listOpen.append(data)

    def getOpenPos(self):
        return self.listOpen

    def getBetResult(self):
        return self.df_martingale

    def updateBetResult(self, br):
        return self.currbetresult

    def sendtoServer(self, txt):        
        r = requests.post('{}'.format(self.url_server), json=txt)
        if r.status_code == 200:
            print("send data to server done.")
